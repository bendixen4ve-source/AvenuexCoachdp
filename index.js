...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    // ---------- PLATFORM ADD ----------
    if (['ytshorts_up', 'reels_up', 'tiktok_up'].includes(customId)) {
      const type = customId.replace('_up', '');

      const modal = new ModalBuilder()
        .setCustomId(`modal_add_${type}`)
        .setTitle(`Tilføj ${getPlatformLabel(type)} upload`);
    ...
    // ---------- PLATFORM REMOVE ----------
    if (['ytshorts_down', 'reels_down', 'tiktok_down'].includes(customId)) {
      ...
      return interaction.reply({
        content: `🗑️ Vælg hvilken ${getPlatformLabel(type)} upload der skal fjernes:`,
        components: [row],
        ephemeral: true
      });
    }
    ...
    // ---------- PLATFORM RESET ----------
    if (['ytshorts_reset_confirm', 'reels_reset_confirm', 'tiktok_reset_confirm'].includes(customId)) {
      const type = customId.replace('_reset_confirm', '');
      return interaction.reply({
        content: `⚠️ Er du sikker på at du vil **nulstille ${getPlatformLabel(type)} trackeren**?`,
